...
class RsFilm final : public Film<Float, Spectrum> {
public:
    ...
    RsFilm(const Properties &props) : Base(props) {
        ...
        std::vector<ScalarFloat> nodevec(m_srfs.size());
        uint32_t i = 0;
        for (auto srf : m_srfs) {
            nodevec[i]  = (ScalarFloat) srf->wavelength_range().x();
            i++;
        }
        compute_srf_sampling(nodevec.data());
    }
    ...
    void compute_srf_sampling(const ScalarFloat *nodesvec) {
       
        FloatStorage nodes = dr::load<FloatStorage>(nodesvec, m_srfs.size());
         // dr::zeros<FloatStorage>(m_srfs.size());
        FloatStorage values = dr::full<FloatStorage>(1.0f,m_srfs.size());
        //Mask mask           = true;
        /* for (auto srf : m_srfs) {
            //nodes +=  srf->wavelength_range().x();
            //values  = 1.0f;
            dr::scatter<FloatStorage>(nodes, srf->wavelength_range().x(), (Index)0);
        }*/

        //Log(Debug, "vector1 cal end!");
        /* auto nodeptr   = nodes.data();
        auto valueptr = values.data();
        for (auto srf : m_srfs) {
            *nodeptr++ =  srf->wavelength_range().x();
            *valueptr++ =  1.0f;
        }*/
        

        using DoubleStorage = dr::float64_array_t<FloatStorage>;
        DoubleStorage nodes_dbl = DoubleStorage(nodes);
        DoubleStorage values_dbl = DoubleStorage(values);

        auto && node_storage = dr::migrate(nodes_dbl, AllocType::Host);
        auto && value_storage = dr::migrate(values_dbl, AllocType::Host);
        if constexpr (dr::is_jit_v<Float>)
            dr::sync_thread();

        // Create new spectrum with the sampling information
        auto props = Properties("discret");
        props.set_pointer("wavelengths", node_storage.data());
        props.set_pointer("values", value_storage.data());
        props.set_long("size", m_srfs.size());
...
MI_IMPLEMENT_CLASS_VARIANT(RsFilm, Film)
MI_EXPORT_PLUGIN(RsFilm, "Remote Sensing Spectral Bands Film")
