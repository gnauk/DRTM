...
public:
    MI_IMPORT_BASE(Shape, m_id, m_to_world, m_to_object, m_emitter, m_shape_type,
                   mark_dirty)
    MI_IMPORT_TYPES(BSDF, Emitter)
    ...
    Instance(const Properties &props) : Base(props) {
        instance_id = props.get<int>("instanceid", 0);
        for (auto &kv : props.objects()) {
            Base *shape = dynamic_cast<Base *>(kv.second.get());
            if (shape && shape->is_shapegroup()) {
                if (m_shapegroup)
                    Throw("Only a single shapegroup can be specified per "
                          "instance.");
        ...
        if (m_shapegroup->is_emitter()) {
            m_emitter = (Emitter *) (m_shapegroup->emitter());
        }
    ...
    SurfaceInteraction3f compute_surface_interaction(const Ray3f &ray,
        ...
        si.instance = this;
        si.instance_index = this->instance_id;
