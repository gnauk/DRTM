...
Bitmap texture (:monosp:`bitmap_polygon`)
---------------------------------
 ...
 * - filter_type
   ...
   - Specifies how pixel values are interpolated and filtered when queried over larger
     ...
     - ``bilinear`` : perform bilinear interpolation, but no filtering.

     - ``nearest``(default): disable filtering and interpolation. In this mode, the plugin
...
bayer = dr.full(mi.TensorXf,0.5, (32,2,1))
bayer[0,0,0 ] = 10
bayer[1,0,0 ] = 30
bayer[2,0,0 ] = 60
bayer[3,0,0 ] = 90
...
si.prim_index = 5
f = heightmap_texture.eval_1(si)
print(f)
si.prim_index = 3
f = heightmap_texture.eval(si)
print(f)
si.prim_index = 2
f = heightmap_texture.eval(si)
print(f)
...
class BitmapPolygonTexture final : public Texture<Float, Spectrum> {
public:
    MI_IMPORT_TYPES(Texture)

    BitmapPolygonTexture(const Properties &props) : Texture(props) {
        m_transform = props.get<ScalarTransform4f>("to_uv", ScalarTransform4f())
                          .extract();
        if constexpr (!is_monochromatic_v<Spectrum>) {
            Throw("BitmapPolygonTexture only work in mono mode now!");
        }
        ...
        std::string filter_mode_str = props.string("filter_type", "nearest");
...
protected:
    ...
    MI_INLINE Float interpolate_1(const SurfaceInteraction3f &si,
        ...
        if constexpr (!dr::is_array_v<Mask>)
            active = true;
        const size_t *shape = m_texture.shape();
        //int hang = si.prim_index % (int)(shape[1]);
        //int lie = si.prim_index / (int)(shape[1]);
        Float out;
        //std::cout << si.prim_index % (int) (shape[1]) * 1.f / (int) (shape[1]) << std::endl;
        //std::cout << si.prim_index* 1.f  / (int) (shape[1]) * 1.f / (int) (shape[0]) + 0.000001f << std::endl;
        if (m_accel)
            m_texture.eval(Point2f(si.prim_index % (int) (shape[1]) * 1.f/ (int) (shape[1]),
                                   (si.prim_index* 1.f  / (int) (shape[1]) * 1.f / (int) (shape[0]) + 0.000001f)),
                &out, active);
        else
            m_texture.eval_nonaccel(
                Point2f(
                    si.prim_index % (int) (shape[1]) * 1.f/ (int) (shape[1]),
                    (si.prim_index* 1.f  / (int) (shape[1]) * 1.f / (int) (shape[0]) +
                     0.000001f)),
                 &out, active);
    ...
    MI_INLINE void init_distr() const {
        ...
        if (!m_distr2d) {
            auto self = const_cast<BitmapPolygonTexture *>(this);
...
MI_IMPLEMENT_CLASS_VARIANT(BitmapPolygonTexture, Texture)
MI_EXPORT_PLUGIN(BitmapPolygonTexture, "Bitmap polygon texture")
