...
class BitmapInstanceTexture final : public Texture<Float, Spectrum> {
public:
    MI_IMPORT_TYPES(Texture, Shape)

    BitmapInstanceTexture(const Properties &props) : Texture(props) {
        m_transform = props.get<ScalarTransform4f>("to_uv", ScalarTransform4f())
                          .extract();
        if constexpr (!is_monochromatic_v<Spectrum>) {
            Throw("BitmapInstanceTexture only work in mono mode now!");
        }
...
protected:
    ...
    MI_INLINE Float interpolate_1(const SurfaceInteraction3f &si,
        ...
        const size_t *shape = m_texture.shape();
        Float out;
        //trick get instance from m_id
        //std::string m_id = (si.instance)->id();
        /*int pos = m_id.rfind("_");
        if (pos == std::string::npos) {
            Throw("BitmapInstanceTexture only work in instance name like 'XXX_123' !");
        }*/ 
        //std::stoi(m_id.substr(pos+1))
       // std::cout<<si.instance_index<<std::endl;
        if (m_accel)
            m_texture.eval(Point2f(si.instance_index % (int) (shape[1]) * 1.f / (int) (shape[1]),
                       (si.instance_index* 1.f / (int) (shape[1]) * 1.f /(int) (shape[0]) +0.000001f)),&out, active);
        else
            m_texture.eval_nonaccel(Point2f(si.instance_index % (int) (shape[1]) * 1.f / (int) (shape[1]),
                        (si.instance_index* 1.f / (int) (shape[1]) * 1.f /(int) (shape[0]) +0.000001f)), &out, active);
    ...
    MI_INLINE void init_distr() const {
        ...
        if (!m_distr2d) {
            auto self = const_cast<BitmapInstanceTexture *>(this);
...
MI_IMPLEMENT_CLASS_VARIANT(BitmapInstanceTexture, Texture)
MI_EXPORT_PLUGIN(BitmapInstanceTexture, "Bitmap instance texture")
