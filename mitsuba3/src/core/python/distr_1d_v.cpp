...
MI_PY_EXPORT(IrregularDiscretDistribution) {
    MI_PY_IMPORT_TYPES()

    using IrregularDiscretDistribution = mitsuba::IrregularDiscretDistribution<Float>;
    using FloatStorage = DynamicBuffer<Float>;

    MI_PY_STRUCT(IrregularDiscretDistribution, py::module_local())
        .def(py::init<>(), D(IrregularDiscretDistribution))
        .def(py::init<const IrregularDiscretDistribution &>(), "Copy constructor")
        .def(py::init<const FloatStorage &, const FloatStorage &>(),
             "nodes"_a, "pdf"_a, D(IrregularDiscretDistribution, IrregularDiscretDistribution, 2))
        .def("__len__", &IrregularDiscretDistribution::size)
        .def("size", &IrregularDiscretDistribution::size, D(IrregularDiscretDistribution, size))
        .def("empty", &IrregularDiscretDistribution::empty, D(IrregularDiscretDistribution, empty))
        .def("range", py::overload_cast<>(&IrregularDiscretDistribution::range),
             D(IrregularDiscretDistribution, range), py::return_value_policy::reference_internal)
        .def("nodes", py::overload_cast<>(&IrregularDiscretDistribution::nodes),
             D(IrregularDiscretDistribution, nodes), py::return_value_policy::reference_internal)
        .def("pdf", py::overload_cast<>(&IrregularDiscretDistribution::pdf),
             D(IrregularDiscretDistribution, pdf), py::return_value_policy::reference_internal)
        .def("cdf", py::overload_cast<>(&IrregularDiscretDistribution::cdf),
             D(IrregularDiscretDistribution, cdf), py::return_value_policy::reference_internal)
        .def("eval_pdf", &IrregularDiscretDistribution::eval_pdf,
             "x"_a, "active"_a = true, D(IrregularDiscretDistribution, eval_pdf))
        .def("eval_pdf_normalized", &IrregularDiscretDistribution::eval_pdf_normalized,
             "x"_a, "active"_a = true, D(IrregularDiscretDistribution, eval_pdf_normalized))
        .def("eval_cdf", &IrregularDiscretDistribution::eval_cdf,
             "x"_a, "active"_a = true, D(IrregularDiscretDistribution, eval_cdf))
        .def("eval_cdf_normalized", &IrregularDiscretDistribution::eval_cdf_normalized,
             "x"_a, "active"_a = true, D(IrregularDiscretDistribution, eval_cdf_normalized))
        .def_method(IrregularDiscretDistribution, update)
        .def_method(IrregularDiscretDistribution, integral)
        .def_method(IrregularDiscretDistribution, normalization)
        .def_method(IrregularDiscretDistribution, interval_resolution)
        .def_method(IrregularDiscretDistribution, max)
        .def("sample",
            &IrregularDiscretDistribution::sample,
            "value"_a, "active"_a = true, D(IrregularDiscretDistribution, sample))
        .def("sample_pdf",
            &IrregularDiscretDistribution::sample_pdf,
            "value"_a, "active"_a = true, D(IrregularDiscretDistribution, sample_pdf))
        .def_repr(IrregularDiscretDistribution);
}
