...
Irradiance meter (:monosp:`irradiancemeter_sunsky`)
--------------------------------------------
...
.. tabs::
    .. code-tab:: xml
        ...
        <shape type="sphere">
            <sensor type="irradiancemetersunsky">
                <!-- film -->
            </sensor>
        </shape>
...
MI_VARIANT class IrradianceMeterSunsky final : public Sensor<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Sensor, m_film, m_shape, sample_wavelengths)
    MI_IMPORT_TYPES(Shape)

    IrradianceMeterSunsky(const Properties &props) : Base(props) {
    ...
    std::string to_string() const override {
        ...
        oss << "IrradianceMeterSunsky[" << std::endl << "  surface_area = ";
...
MI_IMPLEMENT_CLASS_VARIANT(IrradianceMeterSunsky, Sensor)
MI_EXPORT_PLUGIN(IrradianceMeterSunsky, "IrradianceMeterSunsky");
