...
class ShapeAOVIntegrator final : public SamplingIntegrator<Float, Spectrum> {
public:
    ...
    ShapeAOVIntegrator(const Properties &props) : Base(props),
        m_integrator_aovs_count(0) {
        
        m_sundirection = dr::normalize(props.get<ScalarVector3f>("direction"));
        shapeid        = props.get<ScalarUInt32>("shapeid");
        ...
        for (const std::string &token: tokens) {
            ...
            if (item[1] == "albedo") {
                ...
                m_aov_names.push_back(item[0] + ".I");
            } else if (item[1] == "shadeshape_index") {
                m_aov_types.push_back(Type::ShadeShapeIndex);
                m_aov_names.push_back(item[0] + ".L");
                m_aov_names.push_back(item[0] + ".S");
            } else {
                    ...
                    if constexpr (!dr::is_jit_v<Float>) {
                        ...
                        if (it == shape_to_idx.end()) {
                            ...
                            *aovs++ = Float(0.f);
                            ...
                            if (it->second == shapeid) {
                                Ray3f sun_ray = si.spawn_ray(-m_sundirection);
                                Mask occluded =
                                    scene->ray_test(sun_ray, active);
                                *aovs++ = dr::select(occluded, 0.f, 1.f);
                                *aovs++ = dr::select(occluded, 1.f, 0.f);
                            } else {
                                //second scatter
                                ...
                                auto [bsdf_val, bsdf_pdf, bsdf_sample,
                                      bsdf_weight] =
                                    bsdf->eval_pdf_sample(bsdf_ctx, si, wo,
                                                          sample_1, sample_2);
                                Ray3f ray1 =
                                    si.spawn_ray(si.to_world(bsdf_sample.wo));
                                si = scene->ray_intersect(
                                    ray1, (uint32_t) RayFlags::All, true,
                                    active);
                                ShapePtr target = si.instance;
                                if (!target)
                                    target = si.shape;
                                ...
                                if (it == shape_to_idx.end()) {
                                    ...
                                    *aovs++ = Float(0.f);
                                    ...
                                    if (it->second == shapeid) {
                                        Ray3f sun_ray =
                                            si.spawn_ray(-m_sundirection);
                                        Mask occluded =
                                            scene->ray_test(sun_ray, active);
                                        *aovs++ =
                                            dr::select(occluded, 0.f, 0.03f/3.1415926f);
                                        *aovs++ = dr::select(
                                            occluded, 0.03f / 3.1415926f, 0.f);
                                    } else {
                                        *aovs++ = Float(0.f);
                                        *aovs++ = Float(0.f);
                                    }
    ...
    void traverse(TraversalCallback *callback) override {
        ...
        callback->put_parameter("shapeid", shapeid,
                                +ParamFlags::NonDifferentiable);
    }

    std::string to_string() const override {
        std::ostringstream oss;
        oss << "ShapeAOVIntegrator[" << std::endl
...
private:
    ...
    ScalarVector3f m_sundirection;
};

MI_IMPLEMENT_CLASS_VARIANT(ShapeAOVIntegrator, SamplingIntegrator)
MI_EXPORT_PLUGIN(ShapeAOVIntegrator, "ShapeAOV integrator");
