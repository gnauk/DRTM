...
discret spectrum (:monosp:`discret`)
----------------------------------------
...
class DiscretSpectrum final : public Texture<Float, Spectrum> {
public:
    MI_IMPORT_TYPES(Texture)

public:
    DiscretSpectrum(const Properties &props) : Texture(props) {
        if (props.type("values") == Properties::Type::String) {
            ...
            m_distr = IrregularDiscretDistribution<Wavelength>(
            ...
            // NotImplementedError("DiscretSpectrum Scene/property parsing error");
            // Scene/property parsing is in double precision, cast to single precision depending on variant.
            ...
            if constexpr (std::is_same_v<ScalarFloat, double>) {
                m_distr = IrregularDiscretDistribution<Wavelength>(whl, ptr, size);
            } else {
    ...
    Float mean() const override {
        ...
        return m_distr.integral() / m_distr.size();
    }
    ...
    MI_DECLARE_CLASS()
private:
    IrregularDiscretDistribution<Wavelength> m_distr;
};

MI_IMPLEMENT_CLASS_VARIANT(DiscretSpectrum, Texture)
MI_EXPORT_PLUGIN(DiscretSpectrum, "discret spectrum")
