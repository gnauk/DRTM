...
template <typename Value> struct IrregularDiscretDistribution {
    using Float = std::conditional_t<dr::is_static_array_v<Value>,
                                     dr::value_t<Value>, Value>;
    using FloatStorage = DynamicBuffer<Float>;
    using Index = dr::uint32_array_t<Value>;
...
public:
    /// Create an uninitialized IrregularDiscretDistribution instance
    IrregularDiscretDistribution() { }

    /// Initialize from a given density function discretized on nodes \c nodes
    IrregularDiscretDistribution(const FloatStorage &nodes,
                                    const FloatStorage &pdf)
        : m_nodes(nodes), m_pdf(pdf) {
        update();
    }
    ...
    /// Initialize from a given density function (rvalue version)
    IrregularDiscretDistribution(FloatStorage &&nodes,
                                    FloatStorage &&pdf)
        : m_nodes(std::move(nodes)), m_pdf(std::move(pdf)) {
        update();
    }
    ...
    /// Initialize from a given floating point array
    IrregularDiscretDistribution(const ScalarFloat *nodes,
                                    const ScalarFloat *pdf,
                                    size_t size)
        : m_nodes(dr::load<FloatStorage>(nodes, size)), m_pdf(dr::load<FloatStorage>(pdf, size)) {
    ...
    void update() {
        if (m_pdf.size() != m_nodes.size())
            Throw("IrregularDiscretDistribution: 'pdf' and 'nodes' size mismatch!");
    ...
    Value eval_pdf(Value x, Mask active = true) const {
        ...
        return dr::select(active, y0, 0.f);
    }
    ...
    Value eval_cdf(Value x, Mask active = true) const {
        ...
        Value c0 = dr::gather<Value>(m_cdf,   index, active && index >= 0u);
        return c0;
    }
    ...
    std::pair<Value, Value> sample_pdf(Value value, Mask active = true) const {
        ...
        Value x0 = dr::gather<Value>(m_nodes, index,  active);
        return { x0, eval_pdf_normalized(x0,active)};
    }
...
private:
    void compute_cdf(const ScalarFloat *nodes, const ScalarFloat *pdf, size_t size) {
        ...
        double integral = 0.;
        double prevvalue =0.0; //prev wavelength
        std::vector<ScalarFloat> cdf(size);

        m_max = pdf[0];
        for (size_t i = 0; i < size; ++i) {

            double nodevalue = (double) *nodes++;

            m_interval_size = dr::minimum(m_interval_size, (ScalarFloat) nodevalue - (ScalarFloat) prevvalue);
            prevvalue = nodevalue;

            m_range.x() = dr::minimum(m_range.x(), (ScalarFloat) nodevalue);
            m_range.y() = dr::maximum(m_range.y(), (ScalarFloat) nodevalue);

            double value = (double) *pdf++;
            integral += value;
            cdf[i] = (ScalarFloat) integral;
            m_max = dr::maximum(m_max, (ScalarFloat) value);

            if (value < 0.0) {
                Throw("IrregularDiscretDistribution: entries must be non-negative!");
            } else if (value > 0.0) {
                // Determine the first and last wavelength bin with nonzero density
        ...
        if (dr::any(dr::eq(m_valid, (uint32_t) -1)))
            Throw("IrregularDiscretDistribution: no probability mass found!");
...
template <typename Value>
std::ostream &operator<<(std::ostream &os, const IrregularDiscretDistribution<Value> &distr) {
    os << "IrregularDiscretDistribution[" << std::endl
        << "  size = " << distr.size() << "," << std::endl
        << "  nodes = " << distr.nodes() << "," << std::endl
        << "  integral = " << distr.integral() << "," << std::endl
        << "  pdf = " << distr.pdf() << "," << std::endl
